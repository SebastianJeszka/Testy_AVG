...
} from './query-builder.directives';
import { FormService } from 'src/app/_shared/services/form.service';
import { CustomContextMenuEmitValue, CustomContextMenuItem } from 'src/app/_shared/models/custom-context-menu.model';
...
export class QueryBuilderComponent implements OnInit, OnChanges, ControlValueAccessor, Validator {
  ...
  public defaultOperatorMap: { [key: string]: string[] } = {
    ...
    category: ['=', '!=', 'in', 'not in'],
    boolean: ['='],
    multilevel: ['=', '!=', 'in', 'not in']
  };
  public contextMenuItems: CustomContextMenuItem<string>[] = [];
  public availableTechNames: string[] = [];
  ...
  @Input() emptyMessage: string = 'Zestaw zasad nie może być pusty. Proszę dodać zasadę lub usunąć go całkowicie.';
  ...
  ngOnInit() {
    ...
    this.availableTechNames = this.formService.getAvaliableTechNames();
  }
  ...
  handleContextMenuEvent(event: CustomContextMenuEmitValue<string>, rule) {
    rule[event.model] = (rule[event.model] || '') + event.value;
  }
  ...
  onMultilevelChange(val, rule: Rule) {
    rule.value = val;
    this.changeInput();
  }
  ...
  onKey(event: KeyboardEvent) {
    this.filterFields = this.search((event.target as HTMLInputElement).value);
  }
  ...
  search(value: string) {
    ...
    return this.fields.filter((option) => `${option.tabName} ${option.name}`.toLocaleLowerCase().includes(query));
  }
